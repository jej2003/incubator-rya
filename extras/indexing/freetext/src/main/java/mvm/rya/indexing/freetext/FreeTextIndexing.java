...
public class FreeTextIndexing implements IndexingSPI {
    @Override
    public Configurator getCongigurator() {
        return new Configurator() {
            ...
            public Collection<String> getIndexers(RdfCloudTripleStoreConfiguration conf) {
                if(FreeTextConfig.getUseFreeText(conf)) {
                    if (FreeTextConfig.getUseMongo(conf)) {
                        return Collections.singleton(MongoFreeTextIndexer.class.getName());
                    } else {
                        return Collections.singleton(AccumuloFreeTextIndexer.class.getName());
                    }
                }
            ...
            public Class<? extends ExternalTupleSetProvider> provider() {
                return FreeTextTupleSetProvider.class;
            }
        };
    }
}
