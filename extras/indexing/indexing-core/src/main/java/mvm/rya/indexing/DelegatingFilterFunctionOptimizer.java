...
public class DelegatingFilterFunctionOptimizer implements QueryOptimizer, Configurable {

    private ValueFactory valueFactory = new ValueFactoryImpl();

    private Configuration conf;
    private Map<FUNCTION_TYPE, ExternalTupleSetProvider> externalTupleSetProviders = new HashMap<>();
//    private TemporalIndexer temporalIndexer;
    private boolean init = false;


    public DelegatingFilterFunctionOptimizer() {
    }


    public DelegatingFilterFunctionOptimizer(AccumuloRdfConfiguration conf) throws AccumuloException, AccumuloSecurityException,
            TableNotFoundException, IOException, TableExistsException {
        this.conf = conf;
        init();
    }
    ...
    private void init() {
        if (!init) {
            ...
            for(ExternalTupleSetProvider provider : ConfigUtils.getExternalTupleSetProviders(conf)) {
                provider.configure(conf);
                externalTupleSetProviders.put(provider.getFunctionType(), provider);
            }
    ...
    private class IndexerExprReplacer implements TupleExprReplacer {
        ...
        public List<TupleExpr> createReplacement(TupleExpr org) {
            ...
            if(!externalTupleSetProviders.containsKey(type)) {
                throw new IllegalArgumentException("Incorrect type!");
            }

            ExternalTupleSetProvider provider = externalTupleSetProviders.get(type);
            for (IndexingExpr indx : indxExpr) {
                indexTuples.add(provider.get(indx));
            }
