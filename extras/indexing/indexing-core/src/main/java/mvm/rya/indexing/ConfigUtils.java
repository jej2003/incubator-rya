...
import mvm.rya.indexing.IndexingSPI.Configurator;
...
//    public static final String USE_OPTIMAL_PCJ = "sc.use.optimal.pcj";
    ...
    public static final String EXTERNAL_TUPLE_SET_PROVIDERS = "sc.external.tupleproviders";
    ...
    protected static String getIndexTableName(Configuration conf, String indexTableNameConf, String altSuffix){
...
//
    ...
    public static void setIndexers(RdfCloudTripleStoreConfiguration conf) {
        ...
        Set<String> additionalIndexers = new HashSet<>();
        Set<String> additionalOptimizers = new HashSet<>();
        Set<String> externalProviders = new HashSet<>();

        for(IndexingSPI index : ServiceLoader.load(IndexingSPI.class)) {
            Configurator configurator = index.getCongigurator();
            configurator.configure(conf);
            additionalIndexers.addAll(configurator.getIndexers(conf));
            additionalOptimizers.addAll(configurator.getOptimizers(conf));
            if(configurator.provider() != null) {
                externalProviders.add(configurator.provider().getName());
            }
        }

        conf.setStrings(AccumuloRdfConfiguration.CONF_ADDITIONAL_INDEXERS, additionalIndexers.toArray(new String[additionalIndexers.size()]));
        conf.setStrings(AccumuloRdfConfiguration.CONF_OPTIMIZERS, additionalOptimizers.toArray(new String[additionalOptimizers.size()]));
        conf.setStrings(ConfigUtils.EXTERNAL_TUPLE_SET_PROVIDERS, externalProviders.toArray(new String[externalProviders.size()]));

    }

    public static Collection<ExternalTupleSetProvider> getExternalTupleSetProviders(Configuration conf) {
        return conf.getInstances(EXTERNAL_TUPLE_SET_PROVIDERS, ExternalTupleSetProvider.class);
    }

    public static Collection<RyaSecondaryIndexer> getSecondaryIndexers(Configuration conf) {
        return conf.getInstances(AccumuloRdfConfiguration.CONF_ADDITIONAL_INDEXERS, RyaSecondaryIndexer.class);
    }
