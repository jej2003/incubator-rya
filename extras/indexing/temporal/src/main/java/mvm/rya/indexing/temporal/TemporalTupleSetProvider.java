...
public class TemporalTupleSetProvider implements ExternalTupleSetProvider {
    TemporalIndexer indexer;
    @Override
    public void configure(Configuration conf) {
        if (!ConfigUtils.getUseMongo(conf)) {
            indexer = new AccumuloTemporalIndexer();
    ...
    public FUNCTION_TYPE getFunctionType() {
        return FUNCTION_TYPE.TEMPORAL;
    }

    @Override
    public ExternalTupleSet get(IndexingExpr expr) {
        return new TemporalTupleSet(expr, indexer);
    }
}
